        ...
        <>
            <GuestLayout>
                ...
                <div className="text-center mb-8">
                    <h1 className="text-2xl font-black text-white tracking-tighter uppercase leading-none mb-4 italic">إنشاء هوية</h1>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-16">
                        {userTypes.map((type) => (
                            <button
                                ...
                                className={`relative text-right p-4 rounded-[2.5rem] border transition-all duration-700 flex flex-col gap-5 group overflow-hidden ${data.user_type === type.id
                                    ? 'border-amber-400/40 bg-white/[0.04] shadow-2xl'
                                    : 'border-white/5 bg-white/[0.01] hover:border-white/10'
                                    }`}
                            >
                                {/* Role Icon Wrapper */}
                                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-700 ${data.user_type === type.id ? 'bg-amber-400 text-black scale-110' : 'bg-white/5 text-white/20 group-hover:text-white'
                                    }`}>
                                    {type.icon}
                                </div>
                                <div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                <select
                                    ...
                                    className="w-full  border border-white/5 rounded-[2rem] py-6 px-10 text-lg font-black text-gray-900 dark:text-white focus:outline-none focus:border-amber-400/30 transition-all appearance-none cursor-pointer bg-white/[0.05]"
                                >
                                    <option value="" className=" text-gray-900 dark:text-white">-- اختر المركز الإقليمي --</option>
                                    {branches?.map(branch => <option key={branch.id} value={branch.id} className="text-gray-900">{branch.branch_name} ({branch.location_city})</option>)}
                                </select>
                                <div className="absolute left-6 top-1/2 -translate-y-1/2 pointer-events-none text-gray-900 "><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg></div>
                            </div>
